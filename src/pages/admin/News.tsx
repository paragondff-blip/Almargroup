...
export default function AdminNews() {
  ...
  const handleOpenModal = (item?: NewsItem) => {
    if (item) {
      ...
      setEditingNews(null);
      setFormData({ title: "", date: new Date().toISOString().split('T')[0], description: "", image: "" });
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
         <div>
           <h2 className="text-2xl font-black text-primary uppercase tracking-tighter">News & Events Management</h2>
           <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">Manage corporate news and announcements</p>
         </div>
      ...
      {/* Table implementation similar to Activities */}
            ...
            <tbody>
               {news.map(item => (
                 <tr key={item.id} className="border-b border-gray-100/50 hover:bg-gray-50/50">
                   <td className="p-4 text-sm font-bold text-primary">{item.title}</td>
                   <td className="p-4 text-sm text-gray-500">{item.date}</td>
                   <td className="p-4 text-right">
                     <button onClick={() => handleOpenModal(item)} className="text-gray-400 hover:text-primary transition-colors p-1"><Edit className="h-4 w-4" /></button>
                     <button onClick={() => handleDelete(item.id)} className="text-gray-400 hover:text-red-500 transition-colors p-1"><Trash2 className="h-4 w-4" /></button>
                   </td>
                 </tr>
               ))}
            </tbody>
        ...
        <div className="fixed inset-0 bg-primary/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Image URL</label>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" className="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg font-bold uppercase tracking-widest text-[10px]">Save News</button>
              </div>
            </form>
          </div>
        </div>
